...
module.exports = {
    ...
    async inviteUser(req, res) {
        ...
        for (const member of data.members) {
            ...
            console.log(user);
            let confirmToken = generateConfirmToken({id: user.id, defaultCalendar:user.default_calendar_id, email: user.email, login: user.login, event_id: eventId});
    ...
    async confirmInviteUser(req, res) {
        ...
        let exists = await events_calendars.find({event_id: data.event_id, calendar_id: data.defaultCalendar});
        if (exists.length != 0) {
            res.status(403).json({
                status: 403,
        ...
        await events_calendars.save({event_id: data.event_id, calendar_id: data.defaultCalendar});
        res.sendStatus(200);
    },
    ...
    async deleteEvent(req, res) {
        ...
        if (!isAuthor) {
            await events_calendars.delete({event_id: eventId, calendar_id: user.default_calendar_id});
        }
        else {
