...
const Profile = () => {
    ...
    useEffect(() => {
        if (calendars[0]) {
            const date = new Date();
            axios.get(`/api/calendars/${calendars[0].id}/events?date=${date.toISOString()}&limit=5`)
            .then((res) => {
                setUpcommingEvents(res.data.sort((a, b) =>  new Date(a.start_at) - new Date(b.start_at)));
            })
            .catch((err) => {
                console.error(err);
                alert("Error");
    ...
    return(
        <main>
            <Container maxWidth='xl' sx={{display: 'flex', flexDirection: 'column', justifyContent: 'center', mt: '20px'}}>
                ...
                <div className={styles.user}>
                    <div className={styles.heading}>
                        <IconButton onClick={() => navigate('/')} aria-label='go back' className={styles.backBtn}><ArrowBackIcon/></IconButton>
                        <h1>Profile</h1>
                    </div>
                    {userInfo &&
                        <div className={styles.user_container}>
                            <Avatar src={`http://localhost:8000/`} alt={userInfo.full_name} className={styles.avatar} />
                            <h3>{userInfo.full_name}</h3>
                            <span>@{userInfo.login}</span>
                            <Button variant='contained' onClick={() => modalInfoUser.handleOpen()}>Settings</Button>
                        </div>
                    }
                </div>
